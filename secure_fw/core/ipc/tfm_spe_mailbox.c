                                    ...
                                    int32_t client_id, uint32_t *psa_ret)
{
    ...
    case MAILBOX_PSA_FRAMEWORK_VERSION:
        *psa_ret = tfm_rpc_psa_framework_version(params);
        return MAILBOX_SUCCESS;
    case MAILBOX_PSA_VERSION:
        *psa_ret = tfm_rpc_psa_version(params, NS_CALLER_FLAG);
        return MAILBOX_SUCCESS;
    case MAILBOX_PSA_CONNECT:
        *psa_ret = (uint32_t)tfm_rpc_psa_connect(params, NS_CALLER_FLAG);
        return MAILBOX_SUCCESS;
    case MAILBOX_PSA_CALL:
        *psa_ret = (uint32_t)tfm_rpc_psa_call(params, NS_CALLER_FLAG);
        return MAILBOX_SUCCESS;
    case MAILBOX_PSA_CLOSE:
        tfm_rpc_psa_close(params, NS_CALLER_FLAG);
        return MAILBOX_SUCCESS;
    default:
        return MAILBOX_INVAL_PARAMS;
    }
...
__STATIC_INLINE int32_t check_mailbox_msg(const struct mailbox_msg_t *msg)
{
    /*
     * FIXME
     * Comprehensive check of mailbox msessage content
...
int32_t tfm_mailbox_reply_msg(mailbox_msg_handle_t handle, int32_t reply)
{
     ...
     * in the first slot.
     * When multiple ongoing PSA client calls from NSPE are supported, addtional
     * check might be necessary to avoid spoofing the first slot, depending on
     * the final mailbox implementation.
     */
...
int32_t tfm_mailbox_init(void)
{
    ...
    if (ret != MAILBOX_SUCCESS) {
        tfm_rpc_unregister_ops(&mailbox_rpc_ops);
