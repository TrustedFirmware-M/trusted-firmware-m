...
static enum tfm_plat_err_t is_message_valid_for_lcs(const struct rse_provisioning_authentication_header_t *header,
                                                    enum lcm_lcs_t lcs)
{
    ...
    switch(lcs) {
    case LCM_LCS_CM:
       purpose_mask = RSE_PROVISIONING_AUTH_MSG_VALID_IN_CM_LCS
                      << RSE_PROVISIONING_AUTH_MSG_PURPOSE_LCS_MASK_OFFSET;
       break;
    case LCM_LCS_DM:
       purpose_mask = RSE_PROVISIONING_AUTH_MSG_VALID_IN_DM_LCS
                      << RSE_PROVISIONING_AUTH_MSG_PURPOSE_LCS_MASK_OFFSET;
       break;
    case LCM_LCS_SE:
       purpose_mask = RSE_PROVISIONING_AUTH_MSG_VALID_IN_SE_LCS
                      << RSE_PROVISIONING_AUTH_MSG_PURPOSE_LCS_MASK_OFFSET;
       break;
    case LCM_LCS_RMA:
       purpose_mask = RSE_PROVISIONING_AUTH_MSG_VALID_IN_RMA_LCS
                      << RSE_PROVISIONING_AUTH_MSG_PURPOSE_LCS_MASK_OFFSET;
       break;
    default:
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_LCS);
        return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_LCS;
    }

    if (header->purpose & purpose_mask) {
        return TFM_PLAT_ERR_SUCCESS;
    }

    FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_PURPOSE);
    return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_PURPOSE;
}

static enum tfm_plat_err_t get_tp_mode_for_message(const struct rse_provisioning_authentication_header_t *header,
                                                   enum lcm_tp_mode_t *tp_mode)
{
    enum rse_provisioning_auth_msg_required_tp_mode_config_t tp_mode_config;

    tp_mode_config = (header->purpose >> RSE_PROVISIONING_AUTH_MSG_PURPOSE_TP_MODE_OFFSET) &
                     RSE_PROVISIONING_AUTH_MSG_PURPOSE_TP_MODE_MASK;
    switch(tp_mode_config) {
    case RSE_PROVISIONING_AUTH_MSG_REQUIRES_TP_MODE_VIRGIN:
        *tp_mode = LCM_TP_MODE_VIRGIN;
        return TFM_PLAT_ERR_SUCCESS;
    case RSE_PROVISIONING_AUTH_MSG_REQUIRES_TP_MODE_PCI:
        *tp_mode = LCM_TP_MODE_PCI;
        return TFM_PLAT_ERR_SUCCESS;
    case RSE_PROVISIONING_AUTH_MSG_REQUIRES_TP_MODE_TCI:
        *tp_mode = LCM_TP_MODE_TCI;
        return TFM_PLAT_ERR_SUCCESS;
    default:
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_TP_MODE);
        return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_TP_MODE;
    }
}

static bool is_sp_mode_required_for_message(const struct rse_provisioning_authentication_header_t *header)
{
    return (((header->purpose >> RSE_PROVISIONING_AUTH_MSG_PURPOSE_SP_MODE_OFFSET) &
             RSE_PROVISIONING_AUTH_MSG_PURPOSE_SP_MODE_MASK) ==
            RSE_PROVISIONING_AUTH_MSG_REQUIRES_SP_MODE_ENABLED);
}

static bool is_message_chainloaded(const struct rse_provisioning_authentication_header_t *header)
{
    return (((header->purpose >> RSE_PROVISIONING_AUTH_MSG_PURPOSE_SEQUENCING_OFFSET) &
             RSE_PROVISIONING_AUTH_MSG_PURPOSE_SEQUENCING_MASK) ==
            RSE_PROVISIONING_AUTH_MSG_CHAINED);
}

static inline bool is_message_personalized(const struct rse_provisioning_authentication_header_t *header)
{
    return (((header->metadata >> RSE_PROVISIONING_AUTH_MSG_DETAILS_PERSONALIZATION_OFFSET) &
             RSE_PROVISIONING_AUTH_MSG_DETAILS_PERSONALIZATION_MASK) ==
            RSE_PROVISIONING_AUTH_MSG_TYPE_PERSONALIZED);
}

TEST_STATIC_INLINE bool
message_needs_code_data_decryption(const struct rse_provisioning_authentication_header_t *header)
{
    enum rse_provisioning_auth_msg_code_and_data_decryption_config_t decryption_config =
        (header->metadata >> RSE_PROVISIONING_AUTH_MSG_DETAILS_CODE_DATA_DECRYPTION_OFFSET) &
        RSE_PROVISIONING_AUTH_MSG_DETAILS_CODE_DATA_DECRYPTION_MASK;

    return (decryption_config == RSE_PROVISIONING_AUTH_MSG_CODE_DATA_DECRYPTION_AES);
}

TEST_STATIC_INLINE bool
message_needs_secret_decryption(const struct rse_provisioning_authentication_header_t *header)
{
    enum rse_provisioning_auth_msg_secret_values_decryption_config_t decryption_config =
        (header->metadata >> RSE_PROVISIONING_AUTH_MSG_DETAILS_SECRET_VALUES_DECRYPTION_OFFSET) &
        RSE_PROVISIONING_AUTH_MSG_DETAILS_SECRET_VALUES_DECRYPTION_MASK;
                                            ...
                                            void *values_output, size_t values_output_size)
{
    ...
    memcpy(values_output,
           code_and_data_and_secret_values + header->code_size + header->data_size,
           header->secret_values_size);

    if (alg == PSA_ALG_CCM) {
        status = psa_aead_update_ad(operation, values_output, header->secret_values_size);
        if (status != PSA_SUCCESS) {
            return status;
        }
    }

    SCB_CleanDCache_by_Addr(values_output, header->secret_values_size);
                                          ...
                                          void *values_output, size_t values_output_size)
{
    ...
    status = psa_aead_update(
        operation,
        code_and_data_and_secret_values + header->code_size + header->data_size,
        header->secret_values_size, values_output, values_output_size, &output_length);
    if (status != PSA_SUCCESS) {
        return status;
    } else if (output_length != header->secret_values_size) {
        return PSA_ERROR_BUFFER_TOO_SMALL;
    }
...
static enum tfm_plat_err_t aes_generic_message_operation(psa_algorithm_t alg,
                                                         const struct rse_provisioning_authentication_header_t *header,
                                                         const uint8_t *code_and_data_and_secret_values,
                                                         ...
                                                         setup_aes_key_func_t setup_aes_key)
{
    ...
    size_t actual_blob_size = sizeof(*header) + header->code_size + header->data_size +
                              header->secret_values_size;
    ...
    size_t authed_header_size = sizeof(*header) - authed_header_offset;
    enum kmu_hardware_keyslot_t kmu_slot;
    psa_aead_operation_t operation = psa_aead_operation_init();

    if (header->signature_size < AES_TAG_MAX_LEN) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_TAG_LEN);
        return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_TAG_LEN;
    }

    if (header->code_size > code_output_size) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_CODE_SIZE);
        return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_CODE_SIZE;
    }

    if (header->data_size > data_output_size) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_DATA_SIZE);
        return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_DATA_SIZE;
    }

    if (header->secret_values_size > values_output_size) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_VALUES_SIZE);
        return TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_VALUES_SIZE;
    }

    if (message_needs_code_data_decryption(header)) {
        data_size_to_decrypt += header->code_size;
        data_size_to_decrypt += header->data_size;
    }

    if (message_needs_secret_decryption(header)) {
        data_size_to_decrypt += header->secret_values_size;
    }
     ...
     */
    if (!message_needs_secret_decryption(header) &&
        message_needs_code_data_decryption(header)) {
    ...
    if ((alg == PSA_ALG_CCM) || (data_size_to_decrypt != 0)) {
        ...
        status = psa_aead_update_ad(&operation,
                                    ((uint8_t *)header) + authed_header_offset,
                                    authed_header_size);
        if (status != PSA_SUCCESS) {
    ...
    if (alg == PSA_ALG_CCM) {
        ...
        status = psa_aead_verify(&operation, NULL, dummy, &dummy, (uint8_t *)header->signature,
                                 AES_TAG_MAX_LEN);
        if (status != PSA_SUCCESS) {
...
static fih_ret aes_validate_and_unpack_message(const struct rse_provisioning_authentication_header_t *header,
                                               const uint8_t *code_and_data_and_secret_values,
                                               void *code_output, size_t code_output_size,
                                               void *data_output, size_t data_output_size,
                                               void *values_output, size_t values_output_size,
                                               setup_aes_key_func_t setup_aes_key)
{
    ...
    memcpy(iv, (uint8_t *)header->iv, sizeof(header->iv));

    err = aes_generic_message_operation(PSA_ALG_CCM, header,
                                        code_and_data_and_secret_values,
                                        iv, 8,
                                        code_output, code_output_size,
                                        data_output, data_output_size,
...
static fih_ret aes_decrypt_and_unpack_message(const struct rse_provisioning_authentication_header_t *header,
                                              const uint8_t *code_and_data_and_secret_values,
                                              void *code_output, size_t code_output_size,
                                              void *data_output, size_t data_output_size,
                                              void *values_output, size_t values_output_size,
                                              setup_aes_key_func_t setup_aes_key)
{
    ...
    err = aes_generic_message_operation(PSA_ALG_CTR, header,
                                        code_and_data_and_secret_values,
                                        iv, AES_IV_LEN,
                                        code_output, code_output_size,
                                        data_output, data_output_size,
...
static enum tfm_plat_err_t hash_message(const struct rse_provisioning_authentication_header_t *header,
                                        void *code_output, size_t code_output_size,
                                        void *data_output, size_t data_output_size,
                                        void *values_output, size_t values_output_size,
...
static fih_ret ecdsa_validate_and_unpack_message(const struct rse_provisioning_authentication_header_t *header,
                                                 const uint8_t *code_and_data_and_secret_values,
                                                 void *code_output, size_t code_output_size,
                                                 void *data_output, size_t data_output_size,
                                                 void *values_output, size_t values_output_size,
                                                 ...
                                                 get_rotpk_func_t get_rotpk)
{
    ...
    size_t sig_point_len = header->signature_size / 2;
    size_t hash_size;

    /* FixMe: Check the usage of AES in the ECDSA signed blob */
    FIH_CALL(aes_decrypt_and_unpack_message, fih_rc, header, code_and_data_and_secret_values,
             code_output, code_output_size,
             data_output, data_output_size,
             values_output, values_output_size,
             setup_aes_key);
    ...
    err = get_rotpk(header, &public_key_x, &public_key_x_size, &public_key_y, &public_key_y_size);
    if (err != TFM_PLAT_ERR_SUCCESS) {
        FIH_RET(fih_ret_encode_zero_equality(err));
    }
    ...
    FIH_CALL(bl1_internal_ecdsa_verify, fih_rc, RSE_PROVISIONING_CURVE, public_key_x,
             ...
             (uint32_t *)header->signature, sig_point_len,
             (uint32_t *)(header->signature + sig_point_len), sig_point_len);
    FIH_RET(fih_rc);
}
#endif /* RSE_PROVISIONING_ENABLE_ECDSA_SIGNATURES */

TEST_STATIC fih_ret
validate_and_unpack_message(const struct rse_provisioning_authentication_header_t *header,
...
{
    FIH_DECLARE(fih_rc, FIH_FAILURE);
    enum rse_provisioning_auth_msg_signature_config_t sig_config;
     ...
     */
    if (msg_size < sizeof(*header) || header->code_size > msg_size - sizeof(*header) ||
        header->data_size > (msg_size - sizeof(*header) - header->code_size) ||
        header->secret_values_size !=
            (msg_size - sizeof(*header) - header->code_size - header->data_size)) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_SIZE);
        FIH_RET(fih_ret_encode_zero_equality(TFM_PLAT_ERR_PROVISIONING_AUTH_MSG_INVALID_SIZE));
    }

    sig_config = (header->metadata >> RSE_PROVISIONING_AUTH_MSG_DETAILS_SIGNATURE_OFFSET) &
                 RSE_PROVISIONING_AUTH_MSG_DETAILS_SIGNATURE_MASK;
    switch(sig_config) {
#ifdef RSE_PROVISIONING_ENABLE_AES_SIGNATURES
    case RSE_PROVISIONING_AUTH_MSG_SIGNATURE_KRTL_DERIVATIVE:
        FIH_CALL(aes_validate_and_unpack_message, fih_rc, header,
                 code_and_data_and_secret_values,
                 code_output, code_output_size,
                 data_output, data_output_size,
                 values_output, values_output_size,
    ...
    case RSE_PROVISIONING_AUTH_MSG_SIGNATURE_ROTPK_NOT_IN_ROM:
        FIH_CALL(ecdsa_validate_and_unpack_message, fih_rc, header,
                 code_and_data_and_secret_values,
                 code_output, code_output_size,
                 data_output, data_output_size,
                 values_output, values_output_size,
                                         ...
                                         size_t msg_size, const void *ctx)
{
    ...
    struct default_blob_authenticated_data_handler_ctx_t *blob_ctx = (struct default_blob_authenticated_data_handler_ctx_t *)ctx;
    ...
    err = get_tp_mode_for_message(&blob->header, &blob_tp_mode);
    if (err != TFM_PLAT_ERR_SUCCESS) {
        return err;
    }
    ...
    err = is_message_valid_for_lcs(&blob->header, lcs);
    if (err != TFM_PLAT_ERR_SUCCESS) {
        return err;
    }
    ...
    if (FIH_COND_CHECK(is_sp_mode_required_for_message(&blob->header))) {
    ...
    if (is_sp_mode_required_for_message(&blob->header) != (sp_mode_enabled == LCM_TRUE)) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_INVALID_SP_MODE);
        return TFM_PLAT_ERR_PROVISIONING_INVALID_SP_MODE;
    }

    if (is_message_chainloaded(&blob->header) != blob_ctx->message_is_chainloaded) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_INVALID_MESSAGE_SEQUENCING);
        return TFM_PLAT_ERR_PROVISIONING_INVALID_MESSAGE_SEQUENCING;
    }

    if (is_message_personalized(&blob->header)) {
                                                            ...
                                                            size_t msg_size, const void *ctx)
{
    ...
    err = is_message_valid_for_lcs(&auth_plain->header, lcs);
    if (err != TFM_PLAT_ERR_SUCCESS) {
        return err;
    }

    if (is_message_chainloaded(&auth_plain->header) != auth_plain_ctx->message_is_chainloaded) {
        FATAL_ERR(TFM_PLAT_ERR_PROVISIONING_INVALID_MESSAGE_SEQUENCING);
        return TFM_PLAT_ERR_PROVISIONING_INVALID_MESSAGE_SEQUENCING;
    }
    ...
    FIH_CALL(validate_and_unpack_message, fih_rc, &auth_plain->header,
             (uint8_t *)&auth_plain->plain_data, msg_size,
             NULL,
             0,
             (void * )PROVISIONING_BUNDLE_DATA_START,
             PROVISIONING_BUNDLE_DATA_SIZE,
    ...
    /* Message has been successfully validated, we can now pass the (unpacked)
     * payload through to the default_plain_data_handler
     */
    ...
    return default_plain_data_handler(
            (const struct rse_provisioning_message_plain_t *)PROVISIONING_BUNDLE_DATA_START,
            msg_size - sizeof(auth_plain->header), &plain_data_handler_ctx);
}
