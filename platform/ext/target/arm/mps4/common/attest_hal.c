...
enum tfm_plat_err_t tfm_plat_get_cert_ref(uint32_t *size, uint8_t *buf)
{
    ...
    size_t otp_size;
    size_t copy_size;
    /* The CERT_REF can be 19 bytes long, but only 4-bytes aligned reads allowed
     * from the LCM OTP. The buffer has to be padded because of this. */
    uint8_t padded_buf[32];

    err = tfm_plat_otp_read(PLAT_OTP_ID_CERT_REF, sizeof(padded_buf), padded_buf);
    if(err != TFM_PLAT_ERR_SUCCESS) {
        return err;
    }
    /* Copy back the real part of the read out values */
    memcpy(buf, padded_buf, *size);
