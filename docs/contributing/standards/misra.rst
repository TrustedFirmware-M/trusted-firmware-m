...
The `coverity_check.h` library provides helper macros to document MISRA-C static-analysis findings
in a consistent, reviewable way (line-level deviations, block deviations, and false positives).

In the common framework code, a limited set of MISRA-C rules are intentionally suppressed where
the design requires it (for example, PSA APIs using `void *` and platform/arch interfaces). The
current suppressions focus on rules 2.2, 11.3, 11.5, 11.6, and 17.9, and they are always accompanied
by a justification comment.
